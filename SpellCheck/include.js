    ...
    livespell.spellingProviders.push(this),
        ...
        (this.docFocus = function (index) {
            for (var fieldIds = this.arrCleanFields(), i = 0; i < fieldIds.length; i++) {
                ...
                oField.spellcheckproxy && (oField = oField.spellcheckproxy),
                    i == index ? ((oField.className = "" + oField.className + " livespell_focus_glow"), oField.scrollIntoView(!1)) : (oField.className = oField.className.replace(/livespell_focus_glow/g, ""));
            }
        }),
        ...
        (this.setFormVals = function (CaseSensitive, IgnoreAllCaps, IgnoreNumeric, CheckGrammar) {
            (this.CaseSensitive = CaseSensitive),
                ...
                this.isUniPacked && $Spelling && (($Spelling.CaseSensitive = CaseSensitive), ($Spelling.IgnoreAllCaps = IgnoreAllCaps), ($Spelling.IgnoreNumbers = IgnoreNumeric), ($Spelling.CheckGrammar = CheckGrammar));
        }),
        (this.CheckInWindow = function () {
            ...
            livespell.test.BuggyAjaxInFireFox() ? (Dheight = 325) : livespell.test.IE6() && (Dheight = 333),
                "modal" == wm && window.showModalDialog && canmodal
                    ...
                    : window.open(this.url(), "spelldialog", "width=" + Dwidth + ",height=" + Dheight + ",scrollbars=no,resizable=no;centerscreen=yes;location=no;tolbar=no;menubar=no", !1);
        }),
        (this.url = function () {
        ...
        (this.AjaxSpellCheck = function (input, makeSuggestions) {
            ...
            return allFound
                ...
                : void (input.join
                      ? ((input = input.join(livespell.str.chr(1))), livespell.ajax.send("APISPELLARRAY", input, this.Language, makeSuggestions ? "" : "NOSUGGEST", this.id()))
        ...
        (this.SpellCheckSuggest = function (input) {
            ...
            return (
                ...
                input.join ? ((input = input.join(livespell.str.chr(1))), livespell.ajax.send_sync("APISPELLARRAY", input, this.Language, "", this.id())) : livespell.ajax.send_sync("APISPELL", input, this.Language, "", this.id())
            );
        }),
        (this.BinSpellCheck = function (input) {
            ...
            return (
                ...
                input.join
                    ? ((input = input.join(livespell.str.chr(1))), livespell.ajax.send_sync("APISPELLARRAY", input, this.Language, "NOSUGGEST", this.id()))
                    : livespell.ajax.send_sync("APISPELL", input, this.Language, "NOSUGGEST", this.id())
            );
        }),
            ...
            for (var out = new Array(), j = 0; j < F.length; j++) {
                ...
                if ("ALL" === oid.toUpperCase() || "ENABLED" === oid.toUpperCase())
                    for (found = !0, AF = document.body.getElementsByTagName("*"), i = 0; i < AF.length; i++) {
                        ...
                        "textarea" == nn && livespell.insitu.filterTextAreas(AF[i]) && (("ALL" !== oid.toUpperCase() && (AF[i].disabled || AF[i].readOnly)) || ((out = livespell.array.safepush(out, AF[i])), (found = !0))),
                            "input" == nn && livespell.insitu.filterTextInputs(AF[i]) && (("ALL" !== oid.toUpperCase() && (AF[i].disabled || AF[i].readOnly)) || ((out = livespell.array.safepush(out, AF[i])), (found = !0))),
                            ("div" != nn && "iframe" != nn) || (livespell.insitu.filerEditors(AF[i]) && (("ALL" !== oid.toUpperCase() && (AF[i].disabled || AF[i].readOnly)) || ((out = livespell.array.safepush(out, AF[i])), (found = !0))));
                    }
                if ("TEXTAREAS" === oid.toUpperCase())
                    ...
                    frameindex < document.getElementsByTagName("iframe").length &&
                        ((myFrame = document.getElementsByTagName("iframe")[frameindex]), myFrame.id || (myFrame.id = "livespell_IFRAME_id_" + frameindex), (out = livespell.array.safepush(out, myFrame.id)), (found = !0));
                ...
                if (!found)
                    if (document.getElementById(oid)) {
                        ...
                        oid.id
                            ...
                            : oid.name
                            ? ((oid.id = "livespell____" + oid.name), (out = livespell.array.safepush(out, "livespell____" + oid.name)))
                            : 1 == document.getElementsByName(oid).length && ((document.getElementsByName(oid)[0].id = "livespell____" + oid), (out = livespell.array.safepush(out, "livespell____" + oid)));
            }
...
function JavaScriptSpellCheckObj($setup) {
    (this.DefaultDictionary = "English (International)"),
        ...
        (this.LiveValidateMech4Proxy = function (oField, result) {
            ...
            return (
                "string" == typeof MessageHolder && (MessageHolder = document.getElementById(MessageHolder)),
                !(!MessageHolder || !MessageHolder.id) && ((MessageHolder.style.display = result ? "none" : "inherit"), void oField.setAttribute("isValidated", result))
            );
        }),
...
function setup___livespell() {
    for (var tags = document.getElementsByTagName("script"), foundTag = null, i = tags.length - 1; i >= 0 && !foundTag; i--) {
        ...
        try {
            thisTag.getAttribute("src").toLowerCase().indexOf("include.js") > -1 &&
                (thisTag.getAttribute("src").toLowerCase().indexOf("aspnetspell") > -1 || thisTag.getAttribute("src").toLowerCase().indexOf("spellcheck/") > -1) &&
                (foundTag = thisTag);
        } catch (e) {}
    }
    ...
    (path = path.split("?")[0]),
        ...
        "/" == path && (path = ""),
        "/" != path && "" != path && path && !livespell.installPath ? ((livespell.installPath = path), livespell.lang.load("en")) : livespell.lang.load("en");
    ...
    livespell.test.IE5() || livespell.test.IE6() || livespell.test.IE7() || livespell.test.IE8() ? setInterval(livespell.heartbeat, 1200) : setInterval(livespell.heartbeat, 500),
        ...
        livespell.test.MAC() && (livespell.events.add(document, "keydown", livespell.context.mackeydown, !1), livespell.events.add(document, "keyup", livespell.context.mackeyup, !1));
}
    ...
    var livespell = {
        ...
        testSyncRequest: function (posturl) {
            ...
            return 200 == xhr.status
                ? ("undefined" != typeof window.LIVESPELL_DEBUG_MODE && livespell.ajax.debug("SERVER ERROR:" + xhr.responseText, !1),
                  "no command" ==
                      xhr.responseText
                          .toLowerCase()
                          .replace(/^\s\s*/, "")
                          .replace(/\s\s*$/, ""))
                : ("undefined" != typeof window.LIVESPELL_DEBUG_MODE && livespell.ajax.debug("SERVER CANNOT WORK:" + xhr.responseText, !1), !1);
        },
        addJs: function (js) {
        ...
        ajax: {
                ...
                if (((params += "note=" + note), (params += "&command=" + cmd), (params += "&args=" + args), (params += "&lan=" + lan), (params += "&sender=" + sender), (params += "&settingsfile=" + settingsfile), hasajax)) {
                    var liveSpellAjaxCallbackhandler = function () {
                        if (4 == xhr.readyState)
                            ...
                            else if ((livespell.rubberRingServerModel != livespell.setrubberRingServerModel() && livespell.ajax.resend(), "undefined" != typeof window.LIVESPELL_DEBUG_MODE))
                                try {
            ...
            pickup: function (strHTML, binInstant) {
                ...
                if (
                    (livespell.cache.suggestions[vLang] || (livespell.cache.suggestions[vLang] = []),
                    livespell.cache.spell[vLang] || (livespell.cache.spell[vLang] = []),
                    livespell.cache.reason[vLang] || (livespell.cache.reason[vLang] = []),
                    "CTXSPELL" === command)
                ) {
                    for (t = arrResult[2].split(""), r = arrResult[3].split(""), i = 0; i < t.length; i++)
                        (livespell.cache.reason[vLang][livespell.cache.wordlist[vSender][i]] = r[i].toString()), (livespell.cache.spell[vLang][livespell.cache.wordlist[vSender][i]] = "T" === t[i]);
        ...
        cache: { ignore: [], spell: [], reason: [], wordlist: [], suglist: [], langs: [], suggestions: [], suggestionrequest: null, checkTimeout: null, checkTimeoutUni: null, ajaxrequest: [] },
        test: {
            HTML: function (str) {
            ...
            isword: function (str) {
                return (
                    !!str.length &&
                    "length" != str &&
                    ((str = str.replace(/&nbsp;/g, " ")), !(str.indexOf("<") > -1 || str.indexOf("&") > -1) && "&nbsp;" != str && !/[®™]+/.test(str) && ((str = str.replace(/&nbsp;/g, " ")), str.toLowerCase() !== str.toUpperCase()))
                );
            },
            ...
            fullyCached: function (word, lang, makeSuggestions) {
                ...
                return (
                    wordSpellCheck !== !0 &&
                        ((result = result && livespell.cache.reason[lang] && "undefined" != typeof livespell.cache.reason[lang][word]),
            ...
            spliceSpans: function (str, pos, add) {
                ...
                for (var arrStr = str.split(""), inHTMLSpan = !1, inHTML = !0, out = "", j = 0, i = 0; i < arrStr.length; i++) {
                    ...
                    try {
                        i < arrStr.length - 5 &&
                            "<" == ch8r &&
                            (("s" == arrStr[i + 1].toLowerCase() && "p" == arrStr[i + 2].toLowerCase() && "a" == arrStr[i + 3].toLowerCase() && "n" == arrStr[i + 4].toLowerCase()) ||
                                ("/" == arrStr[i + 1].toLowerCase() && "s" == arrStr[i + 2].toLowerCase() && "p" == arrStr[i + 3].toLowerCase() && "a" == arrStr[i + 4].toLowerCase() && "n" == arrStr[i + 5].toLowerCase())) &&
                            (inHTMLSpan = !0),
                            "<" == ch8r && (inHTML = !0);
                    } catch (e) {}
        ...
        cookie: {
            ...
            get: function (check_name) {
                for (var a_all_cookies = document.cookie.split(";"), a_temp_cookie = "", cookie_name = "", cookie_value = "", b_cookie_found = !1, i = 0; i < a_all_cookies.length; i++) {
                    if (((a_temp_cookie = a_all_cookies[i].split("=")), (cookie_name = a_temp_cookie[0].replace(/^\s+|\s+$$/g, "")), cookie_name === check_name))
                        return (b_cookie_found = !0), a_temp_cookie.length > 1 && (cookie_value = unescape(a_temp_cookie[1].replace(/^\s+|\s+$$/g, ""))), cookie_value ? cookie_value : "";
    ...
    Array.push ||
        ...
        (livespell.insitu = {
            ...
            extractTextWithWhitespace: function (parent) {
                for (var elem, ret = "", elems = parent.childNodes, i = 0; elems[i]; i++) {
                    ...
                    3 === elem.nodeType || 4 === elem.nodeType
                        ? (ret += elem.nodeValue)
                        : 1 === elem.nodeType && ((ret += livespell.insitu.extractTextWithWhitespace(elem)), ("P" != elem.nodeName && "DIV" != elem.nodeName) || (ret += "\n"), "BR" == elem.nodeName && elem.nextSibling && (ret += "\n"));
                }
                ...
                if ("" !== val && livespell.insitu.getProxyHTML(id) !== val) {
                    ...
                    if (window.getSelection) {
                        ...
                        if (((clickx = cursorPos.boundingLeft), (clicky = cursorPos.boundingTop), !clickx || !clicky)) {
                            ...
                            (clickx = tempbuddy.getBoundingClientRect().left), (clicky = tempbuddy.getBoundingClientRect().top);
                        }
                        try {
            ...
            checkNow: function (fieldList, providerID) {
                ...
                if ("" == txt) {
                    livespell.cache.spell[this.provider(providerID).Language] || ((livespell.cache.spell[this.provider(providerID).Language] = []), (livespell.cache.reason[this.provider(providerID).Language] = [])),
                    ...
                    for (var mem_words = [], f = 0; f < fieldList.length; f++) {
                        ...
                        if ("textarea" == E$(id).nodeName.toLowerCase() || ("input" == E$(id).nodeName.toLowerCase() && "text" == E$(id).type)) {
                            ...
                            for (
                                var strDoc = this.getProxyText(id), tokens = livespell.str.tokenize(strDoc), lng = this.provider(providerID).Language, memsize = 0, memmax = livespell.maxURI - 257, i = 0;
                    ...
                    if (e && !e.disabled) {
                        ...
                        if (window.getComputedStyle) {
                            ...
                            for (n.oneline || (n.style.overflowY = "scroll"), i = 0; i < stylesToCopy.length; i++)
                                if (((attr = stylesToSet[i]), (styleval = t.currentStyle[attr])))
                                    try {
                                        if ("width" == attr) {
                                            try {
            ...
            cloneClientEvents: function (from, to) {
                for (
                    var clientevents = ["onblur", "onfocus", "onscroll", "onclick", "ondblclick", "ondragstart", "onkeydown", "onkeypress", "onkeyup", "onmousedown", "onmousemove", "onmouseout", "onmouseover", "onmouseup"], k = 0;
                    ...
                    for (var resent = !1, j = 0; j < fieldList.length; j++) {
                        ...
                        if (livespell.insitu.proxyDOM(id)) {
                            for (
                                var token,
                                    show_error,
                                    strDoc = livespell.str.stripSpans(livespell.insitu.getProxyHTML(id)),
                                    tokens = livespell.str.tokenize(strDoc),
                                ...
                                if (tokens_isword[i]) {
                                    ...
                                    1 != livespell.test.spelling(token, this.provider(providerID).Language) && (show_error = !0),
                                        show_error &&
                                            (this.provider(providerID).IgnoreAllCaps && token === token.toUpperCase() && "B" !== reason && "E" !== reason && (show_error = !1),
                                        ...
                                        !show_error &&
                                            ...
                                            ((!show_error && strDoc.indexOf(".") > 0) || strDoc.indexOf("!") > 0 || strDoc.indexOf("?") > 0 || strDoc.length > 50) &&
            ...
            filterTextInputs: function (Area) {
                return (
                    ...
                    ("text" !== Area.type.toLowerCase() ||
                        ...
                        (Area.id || (Area.id = "livespell__input__" + livespell.insitu.idcount++), (Area.hasSpellProxy = !0)),
                    Area.hasSpellProxy)
                );
            },
            filerEditors: function (elem) {
                if ("div" == elem.nodeName.toLowerCase()) {
                    ...
                    if (
                        mydiv &&
                        (1 == mydiv.getAttribute("contenteditable") || "true" === mydiv.getAttribute("contenteditable")) &&
                        (mydiv.id || (mydiv.id = "livespell_rich_div_editor_id_" + livespell.insitu.idcount++), mydiv.id.indexOf("__livespell_proxy") == -1)
                    )
                        return !0;
                }
            ...
            ignoreAtCursor: function () {
                ...
                try {
                    window.getSelection
                        ? ((target = window.getSelection().focusNode), "SPAN" !== target.nodeName.toUpperCase() && (target = target.parentNode))
                        : document.selection && (target = document.selection.createRange().parentElement());
                } catch (e) {}
            ...
            typoclick: function (event, oparent, ospan, reason, providerID) {
                ...
                return (
                    ...
                    event.pageX || event.pageY
                        ...
                        : (event.clientX || event.clientY) && ((posx = event.clientX + document.body.scrollLeft + document.documentElement.scrollLeft), (posy = event.clientY + document.body.scrollTop + document.documentElement.scrollTop)),
                    (posx += 2),
                    (posy += 2),
        ...
        (livespell.context = {
            ...
            showMenu: function (id, word, reason, providerID) {
                ...
                switch (reason) {
                    ...
                    default:
                        ...
                        if (2 === oCase) for (j = 0; j < suggs.length; j++) suggs[j] = suggs[j].toUpperCase();
                        else if (1 === oCase) for (j = 0; j < suggs.length; j++) suggs[j] = livespell.str.toCaps(suggs[j]);
                }
                (suggs.length && 0 !== suggs[0].length) || ((suggs = []), (suggs[0] = livespell.lang.fetch(providerID, "MENU_NOSUGGESTIONS")), (action = "IGNORE")),
                    "X" === reason && ((action = "REG"), livespell.spellingProviders[providerID].isUniPacked && (suggs = new Array("JavaScriptSpellCheck", "**Trial**", "Please register online", "javascriptspellcheck.com")));
                var dsuggs = [];
            ...
            render: function (suggs, action, providerID, reason) {
                    ...
                    switch (action) {
                        ...
                        case "REG":
                            livespell.spellingProviders[providerID].isUniPacked
                                ...
                                : "asp.net" == livespell.spellingProviders[providerID].ServerModel.toLowerCase() || "aspx" == livespell.spellingProviders[providerID].ServerModel.toLowerCase()
                                ? ((menuHTML += '<li class="ls_sug" ><a href="#"  onclick="window.open(\'http://www.aspnetspell.com/Purchase\');return false;" >ASPNetSpell Trial</a></li>'), (suggs = [""]))
                                : "asp" == livespell.spellingProviders[providerID].ServerModel.toLowerCase()
                                ? ((menuHTML += '<li class="ls_sug" ><a href="#"" onclick="window.open(\'http://www.aspspellcheck.com/purchase\');return false;" >ASPSpellCheck Trial</a></li>'), (suggs = [""]))
                                : ((menuHTML += '<li class="ls_sug" ><a href="#" onclick="window.open(\'http://www.phpspellcheck.com/Purchase\');return false;" >PHPSpellCheck Trial</a></li>'), (suggs = [""]));
                            break;
                        case "DELETE":
                ...
                if (
                    ...
                    this.buttonIsHidden("menuAddToDict", providerID) ||
                        "B" == reason ||
                        "E" == reason ||
                        "NONE" == livespell.spellingProviders[providerID].AddWordsToDictionary ||
                    ...
                    if (
                        (1 == livespell.MultipleDictionaries && livespell.MultipleDictionaries && (doMultipleDict = !0),
                        livespell.spellingProviders[providerID].Language.indexOf(",") > 0 && ((livespell.MultipleDictionaries = !0), (doMultipleDict = !0)),
                        doMultipleDict)
                    ) {
                        for (
            ...
            suggest: function (id, word, reason, caller) {
                ...
                return (
                    ...
                    livespell.cache.suggestions[Lang] && livespell.cache.suggestions[Lang][word]
