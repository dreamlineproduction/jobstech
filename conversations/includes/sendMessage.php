<script src="https://cdn.tiny.cloud/1/ouutnajyw0yyfzhie2ln4vsalvch8tdj3fgfquelznqr2yqv/tinymce/5/tinymce.min.js"
...
<div class="sendMessage pl-3 pr-3 pb-3">
    <p class="bg-danger p-2 text-white mv-0 d-none"><i class="fa fa-warning"></i> You seem to have typed word(s) that
        are in violation of our policy. No direct payments or emails allowed.</p>
    <?php if($seller_vacation == "on" AND $seller_status != "block-ban"){ ?>
    <div class="alert alert-info mt-2">
        <div id="seller-vacation-div" class="mb-0">
            <p class="lead mb-0"><strong>Opps! </strong> This seller is on vacation and is not receiving messages at the
                momment. Please try again later.</p>
        </div>
    </div>
    <?php } ?>
    ...
    <form id="insert-message-form">
        ...
        <textarea class="form-control mb-2 text" rows="2" id="message"
            placeholder="<?= $lang['placeholder']['type_message']; ?>"></textarea>
        ...
        <button type="button" class="btn btn-outline-success arrow-drop float-right" data-html="true"
