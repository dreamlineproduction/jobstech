...
<small class="form-text text-danger"><?= ucfirst(
    @$form_errors["proposal_description"]
) ?></small>
...
while ($row_delivery_times = $get_delivery_times->fetch()) {
    ...
    ?>
<option value="<?= $delivery_id ?>" <?php if (
    @$form_data["delivery_id"] == $delivery_proposal_title
) {
    echo "selected";
...
<div class="form-group row d-none"><!--- form-group row Starts --->
  ...
  <small class="form-text text-danger"><?= ucfirst(
      @$form_errors["proposal_revisions"]
  ) ?></small> -->
</div><!--- form-group row Ends --->
...
<small class="form-text text-danger"><?= ucfirst(
    @$form_errors["proposal_enable_referrals"]
) ?></small>
</div>
</div><!--- form-group row Ends --->
...
<div class="col-md-9">
<input type="number" name="proposal_referral_money" class="form-control" min="1" value="<?= @$form_data[
    "proposal_referral_money"
] ?>" placeholder="Figure should be in percentage e.g 20">
...
<div class="col-md-9">
  ...
  <small class="form-text text-danger"><?= ucfirst(
      @$form_errors["proposal_tags"]
  ) ?></small>
</div>
</div><!--- form-group row Ends --->
<div class="form-group text-right mb-0"><!--- form-group Starts --->
  <a href="view_proposals" class="btn btn-secondary"><?= $lang["button"][
      "cancel"
  ] ?></a>
  <input class="btn btn-success" type="submit" name="submit" value="<?= $lang[
      "button"
  ]["save_continue"] ?>">
...
{
    global $db;
    $insertPackage1 = $db->insert("proposal_packages", [
        "proposal_id" => $proposal_id,
        "package_name" => "Basic",
        "price" => 5,
    ]);
    $insertPackage2 = $db->insert("proposal_packages", [
        "proposal_id" => $proposal_id,
        "package_name" => "Standard",
        "price" => 10,
    ]);
    $insertPackage3 = $db->insert("proposal_packages", [
        "proposal_id" => $proposal_id,
...
if (isset($_POST["submit"])) {
    ...
    $messages = [
        ...
        "proposal_enable_referrals" =>
            "you must need to enable or disable proposal referrals.",
    ...
    if ($val->run() == false) {
        ...
        $select_proposals = $db->select("proposals", [
            "proposal_seller_id" => $login_seller_id,
            "proposal_url" => $sanitize_url,
        ]);
      ...
      text: 'Opps! Your Already Made A Proposal With Same Title Try Another.',
            ...
            $data["delivery_id"] = $db
                ->query("select * from delivery_times")
                ->fetch()->delivery_id;
