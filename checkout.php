...
if (isset($_POST['add_order']) or isset($_POST['coupon_submit'])) {
    ...
    if (isset($_SESSION['c_proposal_id'])) {
        ...
        if (isset($_POST['class_date'])) {
            $_SESSION['c_class_date'] = date('Y-m-d', strtotime($input->post('class_date')));
            $class_date = $input->post('class_date');
            $_SESSION['c_class_time'] = $input->post('class_time');
...
Please confirm your email to use this feature.
                                    ...
                                    <div class="card-body">
                                        ...
                                        <?php if ($enable_stripe == "yes") { ?>
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-11">
                                                    <img src="images/credit_cards.jpg" height="30"
                    ...
                    <div class="col-md-5">
                        <div class="card shadow-sm checkout-details">
                            ...
                            <div class="card-body">
                                ...
                                <h6>Proposal's Price: <span class="float-right"><?= showPrice($single_price); ?> </span>
                                </h6>

                                <?php if (isset($_POST['proposal_extras'])) { ?>
                                    <hr>
                                    <h6>Proposal's Extras : <span
                                                class="float-right"><?= showPrice($extra_price); ?></span></h6>
                                <?php } ?>
                                ...
                                <?php if (isset($_SESSION['c_proposal_minutes'])) { ?>
                                    <hr>
                                    <h6>Proposal's Video Call Minutes: <span
                                                class="float-right"><?= $_SESSION['c_proposal_minutes']; ?> Minutes</span>
                                    </h6>
                                <?php } ?>
                                <hr class="processing-fee">
                                <h6 class="processing-fee">Processing Fee: <span
                                            class="float-right"><?= showPrice($processing_fee); ?></span></h6>

                                <?php if (isset($numberToAdd) and $coupon_usage == "used") { ?>
                                    <hr>
                                    <h6>Coupon Discount : <span
                                                class="float-right"><?= showPrice($numberToAdd); ?></span></h6>
                                <?php } ?>
                                ...
                                <form class="input-group" method="post">
                                    ...
                                    <?php if (isset($_POST['proposal_extras'])) { ?>
                                        <input type="hidden" name="proposal_price"
                                               value="<?= $proposal_price - $extra_price; ?>">
                                    <?php } else { ?>
                                    ...
                                    <?php if (isset($_POST['package_id'])) { ?>
                                        <input type="hidden" name="package_id"
                                               value="<?= $input->post('package_id'); ?>">
                                    <?php } ?>
                                    <?php if (isset($_POST['proposal_extras'])) { ?>
                                        <input type="hidden" name="proposal_extras"
                                               value="<?= base64_encode(serialize($proposal_extras)); ?>">
                                    <?php } ?>
                                    <input type="text" name="code" class="form-control apply-disabled"
                                           placeholder="Enter Coupon Code">
                                    <button type="submit" name="coupon_submit" class="input-group-addon btn">Apply
                                    </button>
                                </form>
                                ...
                                <h5 class="font-weight-bold">
                                    Proposal's Total: <span
                                            class="float-right total-price"><?= showPrice($total); ?></span>
                                </h5>
                                <hr>
    ...
    if ($count_cart == 1) {
        ...
        if ($videoPlugin == 1) {
            $insert_cart = $db->insert("cart", array("seller_id" => $login_seller_id, "proposal_id" => $proposal_id, "proposal_price" => $proposal_price, "proposal_qty" => $proposal_qty, "delivery_id" => $delivery_id, "revisions" => $revisions, "video" => $video, "class_date" => $class_date, "class_time" => $class_time));
        } else {
