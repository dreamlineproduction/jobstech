<div class="mp-gig-top-nav">
    <nav>
        <ul class="container text-center" id="mainNav">
            ...
            <li>
                <a href="#redirect"
                    onclick="window.location.href='../../conversations/message.php?seller_id=<?= $proposal_seller_id; ?>'"
...
<div class="container mt-5" id="introduction">
    ...
    <div class="row">
        ...
        ?>
                        <span class="text-muted span"> (<?= $count_reviews; ?>) &nbsp;<?= $proposal_order_queue; ?>
                            Order(s) In Queue.</span>
                        <div class="sharethis-inline-share-buttons <?=($lang_dir == "right" ? 'float-left':'')?>"
                            style="margin-top: -36px;"></div>
                    </div>
                                    ...
                                    <td>
                                        <form method="post" action="../../checkout">
                                            ...
                                            <a class="btn btn-success text-white btn-block"
                                                href="../edit_proposal?proposal_id=<?= $proposal_id; ?>&pricing">Edit
                                                Package</a>
                                            <?php }else{ ?>
                                            <button class="btn btn-success text-white btn-block" type="submit"
                                                name="add_order" <?= $disabled; ?>>Select
                                                <?= showPrice($p_price_1); ?></button>
                                            <?php } ?>
                                        </form>
                                    </td>
                                    <td>
...
$(document).ready(function() {
    ...
    $(".order-now").click(function() {
        $('<input />').attr('type', 'hidden').attr('name', "add_order").attr('value', " ").appendTo(
            form);
        $(form).submit();
    });

    $(".add-to-cart").click(function() {
        $('<input />').attr('type', 'hidden').attr('name', "add_cart").attr('value', " ").appendTo(
            form);
        $(form).submit();
    });
    ...
    $(document).on("click", "#favorite_<?= $proposal_id; ?>", function(event) {
        ...
        $.ajax({
            ...
            success: function() {
                ...
                $("#unfavorite_<?= $proposal_id; ?>").html(
                    "<i class='fa fa-heart dil'></i>");
            }
        });
    });
    ...
    $(document).on("click", "#unfavorite_<?= $proposal_id; ?>", function(event) {
        ...
        $.ajax({
            ...
            success: function() {
                ...
                $("#favorite_<?= $proposal_id; ?>").html(
                    "<i class='fa fa-heart dil1'></i>");
            }
        });
    });
    <?php } ?>
